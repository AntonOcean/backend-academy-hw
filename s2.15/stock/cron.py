...
class MQ:
    ...
    def callback(self, ch, method, properties, body: bytes) -> None:
        ...
        stock = session.query(Stock).filter(Stock.item_id == item_id).one_or_none()
        if not stock:
            self.produce(exchange='', routing_key='stock_fail', body=order_id)
            print(f'[*] Produce to `stock_fail` -> `{order_id}`')
            return
